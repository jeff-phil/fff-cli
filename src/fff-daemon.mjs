...
function showHelp(exitCode = 0) {
  ...
  sink('  --log-level <level>               trace | debug | info | warn | error');
  sink('  --cache-budget-max-files <n>      Cache file-count cap (0 = auto)');
  sink('  --cache-budget-max-bytes <n>      Cache byte cap (0 = auto)');
  sink('  --cache-budget-max-file-size <n>  Cache per-file byte cap (0 = auto)');
  sink('  --disable-content-indexing        Reduces memory but cripples ffgrep');
  sink(
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--cache-budget-max-files': {
        ...
        if (Number.isNaN(n)) {
          console.error(`${NAME}: --cache-budget-max-files requires a number`);
          process.exit(1);
        }
      ...
      case '--cache-budget-max-bytes': {
        const n = parseInt(argv[++i], 10);
        if (Number.isNaN(n)) {
          console.error(`${NAME}: --cache-budget-max-bytes requires a number`);
          process.exit(1);
        }
      ...
      case '--cache-budget-max-file-size': {
        const n = parseInt(argv[++i], 10);
        if (Number.isNaN(n)) {
          console.error(`${NAME}: --cache-budget-max-file-size requires a number`);
          process.exit(1);
        }
...
function buildFinderOpts(disableWatch) {
  ...
  if (args.cacheBudgetMaxFiles > 0) opts.cacheBudgetMaxFiles = args.cacheBudgetMaxFiles;
  if (args.cacheBudgetMaxBytes > 0) opts.cacheBudgetMaxBytes = args.cacheBudgetMaxBytes;
...
const handlers = {
  ...
  async grep({
    ...
    const r = finder.grep(query, {
      mode,
      smartCase,
      maxMatchesPerFile: Math.min(Math.max(1, maxMatchesPerFile || 50), 100),
      pageSize,
